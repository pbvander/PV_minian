...
%%
function msRun_mc_only_python(dpath, vid_path, isnonrigid, chk_size, grid_size, spatial_downsampling, bin_width, mot_uf, correct_bidir, overlap_pre, overlap_post, max_shift);
...
%% Subset data into chunks (much faster if directory contains lots of video files)
files = dir([filepath '*.avi']);
for i=1:length(files)
    [~,name,~] = fileparts(files(i).name);
    files(i).vid_i = str2double(name);
...
for chk = 1:length(unique([files.chunk]))
    ...
    disp("Videos " + vids(1,1) + " - " + vids(1, length(vids)))
    %% 1 - Create video object and save into matfile
    disp('Step 1: Create video object');
    ms = msGenerateVideoObj_chunk(filepath, chk, files);
    ...
    writetable(cell2table(motion), strjoin([vid_path_split(1:length(vid_path_split)-1) ['motion', num2str(chk), '.csv']],"\\"), "WriteVariableNames",false) %write to a csv file
end
