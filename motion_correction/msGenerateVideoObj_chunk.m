function ms = msGenerateVideoObj_chunk(dirName, chk,files)%, filePrefix)
...
%   Detailed explanation goes here
    ...
    %find the total number of relevant video files (PV altered 2/28/23 -
    %added last line and commented out rest (for loop, if statements only
    %needed if fileprefix is "msCam"
...
ms.numFiles = length(files([files.chunk] == chk));
    
    %generate a vidObj for each video file. Also calculate total frames
    for i=0:(round(ms.numFiles-1)) %PV changed this to iterate from 0
%         [folder filesep num2str(filePrefix) num2str(i) '.avi']
        ms.vidObj{i+1} = VideoReader([dirName num2str(i + ((chk-1) * 10)) '.avi']); %PV changed index to i+1
        ms.vidNum = [ms.vidNum i*ones(1,ms.vidObj{i+1}.NumberOfFrames)];
        ms.frameNum = [ms.frameNum 1:ms.vidObj{i+1}.NumberOfFrames];
        ms.numFrames = ms.numFrames + ms.vidObj{i+1}.NumberOfFrames;
    end
